...
class Interpreter:
    ...
    def save_data_to_npy(self):
        ...
        np.save("m87_car_speed.npy", car_speed_npy)
        reel_speed_npy = np.stack(self.reel_speed)
        np.save("m87_reel_speed.npy", reel_speed_npy)
        cb_speed_npy = np.stack(self.cb_speed)
        np.save("m87_cb_speed.npy", cb_speed_npy)
        reel_current_npy = np.stack(self.reel_current)
        np.save("m87_reel_current.npy", reel_current_npy)
        cm7290_current_npy = np.stack(self.cm7290_current)
        np.save("m87_cm7290_current.npy", cm7290_current_npy)
        cb_current_npy = np.stack(self.cb_current)
        np.save("m87_cb_current.npy", cb_current_npy)
