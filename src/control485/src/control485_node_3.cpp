...
const int reelMotor=11,cbMotor=9,pfMotor=10;
string port="/dev/rs485-03";
...
bool openSerial(const char* port)
{
    ...
    timeval time_out;
    time_out.tv_sec=0;
    time_out.tv_usec=1000 * 1000; //设置500ms超时
    ...
    {
        cout<<"Cannot connect modbus-3 at port:"<<port<<endl;
        return false;
    } else
        cout<<"Connected modbus-3 at port:"<<port<<endl;
