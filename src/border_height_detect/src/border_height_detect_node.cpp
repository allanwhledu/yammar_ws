...
ros::Publisher height_border_pub;  //发布分界线,高度信息
ros::Publisher path_track_pub;     // pub the path tracking info
...
const float uncut_plane_height = 0.5;
const float cut_plane_height = 0.35;

const int roi_left_index = 200;
const int roi_right_index = 450;
const int roi_col_offset = 60;
int uncut_plane_count_ema = 0;
int cut_plane_count_ema = 0;
...
int main(int argc,char** argv)
{
    ...
    image_pub= transport.advertise("/height_border_img", 1);
    ...
    path_track_pub = nh.advertise<std_msgs::Int16>("/path_track", 1000);
    ros::spin();
    return 0;
}
...
Eigen::VectorXf boundpoints_clusterd(Mat& rgb, Mat& depth,pcl::PointCloud<pcl::PointXYZRGB>::Ptr& cloudin,pcl::PointCloud<pcl::PointXYZRGB>::Ptr& plane_uncut,vector<Point2i>& pointdepthimg,vector<Point3f>& pointdepthimg_3d)
{
    ...
    for(int row=200;row< rgb.rows;row++)  //ROI遍历范围
        for(int col=roi_left_index  + int((row - 200) * roi_col_offset / 279 ) ;col<roi_right_index -  int((row - 200) * roi_col_offset / 279);col++ )
        {
            ...
            point_plane.x = col;
            point_plane.y = row;
            if(distance > uncut_plane_height && row > 300){
                uncut_plane_count++;
...
//                circle(rgb, pointdepth, 3, Scalar(100, 255, 100));
    ...
    uncut_plane_count_ema = uncut_plane_count_ema == 0 ? uncut_plane_count : 0.8 * uncut_plane_count_ema + 0.2 * uncut_plane_count;
    cut_plane_count_ema = cut_plane_count_ema == 0 ? cut_plane_count : 0.8 * cut_plane_count_ema + 0.2 * cut_plane_count;
